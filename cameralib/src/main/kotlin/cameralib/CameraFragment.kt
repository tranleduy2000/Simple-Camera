...
import cameralib.databinding.CamlibFragmentCameraBinding
...
class CameraFragment : BaseCameraFragment(), CameraXPreviewListener {
    companion object {
        fun newInstance(extras: Bundle?): CameraFragment {
    ...
    private lateinit var binding: CamlibFragmentCameraBinding
    ...
    private val configuration: CameraLibConfiguration by lazy { CameraLibConfiguration.fromBundle(arguments ?: Bundle()) }

    private val tabSelectedListener = object : TabSelectedListener {
        override fun onTabSelected(tab: TabLayout.Tab) {
            handlePermission(Manifest.permission.RECORD_AUDIO) {
                if (it) {
                    when (tab.position) {
                    ...
                    requireContext().toast(R.string.camlib_no_audio_permissions)
                    selectPhotoTab()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = CamlibFragmentCameraBinding.inflate(inflater, container, false);
        return binding.root;
    }
    ...
    override fun onResume() {
        ...
        if (hasStorageAndCameraPermissions()) {
            ...
            mFocusCircleView.setStrokeColor(requireContext().getProperPrimaryColor())
    ...
    private fun initVariables() {
        ...
        mediaSoundHelper = MediaSoundHelper(requireContext())
...
//    }
    ...
    private fun tryInitCamera() {
        handlePermission(Manifest.permission.CAMERA) { grantedCameraPermission ->
            if (grantedCameraPermission) {
                /*handleStoragePermission*/ run {
                    ...
                    if (isInPhotoMode) {
                        ...
                        handlePermission(Manifest.permission.RECORD_AUDIO) { grantedRecordAudioPermission ->
...
//    }
    ...
    private fun isImageCaptureIntent(): Boolean {
        //return intent?.action == MediaStore.ACTION_IMAGE_CAPTURE || intent?.action == MediaStore.ACTION_IMAGE_CAPTURE_SECURE
        return false;
    }
    ...
    private fun createToggleGroup(): MaterialButtonToggleGroup {
        return MaterialButtonToggleGroup(requireContext()).apply {
    ...
    private fun initializeCamera(isInPhotoMode: Boolean) {
        ...
        val outputUri = arguments?.get(MediaStore.EXTRA_OUTPUT) as? Uri
        mPreview = CameraXInitializer(requireActivity()).createCameraXPreview(
            binding.previewView,
            listener = this,
        ...
        mFocusCircleView = FocusCircleView(requireContext()).apply {
    ...
    private fun initModeSwitcher() {
        ...
        val gestureDetector = GestureDetectorCompat(context, object : GestureDetectorListener() {
    ...
    private fun showLastMediaPreview() {
        ...
        if (ON_PREVIEW_THUMB_CLICK != null) {
            ON_PREVIEW_THUMB_CLICK?.invoke(activity, previewUri)
        } else {
            val path = applicationContext.getRealPathFromURI(previewUri) ?: previewUri.toString()
            activity.openPathIntent(path, false)
        }
    }
    ...
    private fun setupOrientationEventListener() {
        mOrientationEventListener = object : OrientationEventListener(requireContext(), SensorManager.SENSOR_DELAY_NORMAL) {
    ...
    private fun setButtonColors(button: MaterialButton) {
        ...
        val iconColors = intArrayOf(ContextCompat.getColor(context, R.color.camlib_md_grey_white), primaryColor)
